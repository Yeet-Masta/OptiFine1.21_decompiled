...
public class ChunkMap extends ChunkStorage implements PlayerProvider, GeneratingChunkMap {
   ...
   public static final int f_143033_ = net.minecraft.server.level.ChunkLevel.m_287154_(FullChunkStatus.ENTITY_TICKING);
   ...
   private final net.minecraft.util.thread.BlockableEventLoop<Runnable> f_140135_;
   ...
   private final net.minecraft.server.level.ChunkMap.DistanceManager f_140145_;
   ...
   private final Int2ObjectMap<net.minecraft.server.level.ChunkMap.TrackedEntity> f_140150_ = new Int2ObjectOpenHashMap();
   ...
   public ChunkMap(
      ...
      Executor executorIn,
      net.minecraft.util.thread.BlockableEventLoop<Runnable> taskExecutorIn,
      ...
      this.f_140145_ = new net.minecraft.server.level.ChunkMap.DistanceManager(executorIn, taskExecutorIn);
   ...
   private static double m_140226_(net.minecraft.world.level.ChunkPos chunkPosIn, Entity entityIn) {
   ...
   boolean m_295978_(ServerPlayer playerIn, int xIn, int zIn) {
      return playerIn.m_292900_().m_294219_(xIn, zIn) && !playerIn.f_8906_.f_290664_.m_296008_(net.minecraft.world.level.ChunkPos.m_45589_(xIn, zIn));
   }
   ...
   public String m_140204_(net.minecraft.world.level.ChunkPos pos) {
   ...
   private CompletableFuture<ChunkResult<List<ChunkAccess>>> m_280541_(ChunkHolder chunkHolderIn, int rangeIn, IntFunction<ChunkStatus> funcStatusIn) {
      if (rangeIn == 0) {
         ...
         net.minecraft.world.level.ChunkPos chunkpos = chunkHolderIn.m_338581_();

         for (int i = -rangeIn; i <= rangeIn; i++) {
            for (int j = -rangeIn; j <= rangeIn; j++) {
               int k = Math.max(Math.abs(j), Math.abs(i));
               long l = net.minecraft.world.level.ChunkPos.m_45589_(chunkpos.f_45578_ + j, chunkpos.f_45579_ + i);
         ...
         return net.minecraft.Util.m_137567_(list).thenApply(chunkResultsIn -> {
   ...
   public ReportedException m_203751_(IllegalStateException exceptionIn, String detailsIn) {
      ...
      net.minecraft.CrashReport crashreport = net.minecraft.CrashReport.m_127521_(exceptionIn, "Chunk loading");
   ...
   ChunkHolder m_140176_(long chunkPosIn, int newLevel, @Nullable ChunkHolder holder, int oldLevel) {
      if (!net.minecraft.server.level.ChunkLevel.m_287217_(oldLevel) && !net.minecraft.server.level.ChunkLevel.m_287217_(newLevel)) {
         return holder;
      } else {
         if (holder != null) {
         ...
         if (holder != null) {
            if (!net.minecraft.server.level.ChunkLevel.m_287217_(newLevel)) {
         ...
         if (net.minecraft.server.level.ChunkLevel.m_287217_(newLevel) && holder == null) {
            holder = (ChunkHolder)this.f_140131_.remove(chunkPosIn);
            if (holder != null) {
               holder.m_140027_(newLevel);
            } else {
               holder = new ChunkHolder(new net.minecraft.world.level.ChunkPos(chunkPosIn), newLevel, this.f_140133_, this.f_140134_, this.f_140141_, this);
            }
   ...
   private CompletableFuture<ChunkAccess> m_140417_(net.minecraft.world.level.ChunkPos chunkPosIn) {
   ...
   private ChunkAccess m_214901_(Throwable throwableIn, net.minecraft.world.level.ChunkPos chunkPosIn) {
      ...
      if (!flag1) {
         ...
         net.minecraft.CrashReport crashreport = net.minecraft.CrashReport.m_127521_(throwableIn, "Exception loading chunk");
   ...
   private ChunkAccess m_214961_(net.minecraft.world.level.ChunkPos chunkPosIn) {
      this.m_140422_(chunkPosIn);
   ...
   private void m_140422_(net.minecraft.world.level.ChunkPos chunkPosIn) {
      this.f_140151_.put(chunkPosIn.m_45588_(), (byte)-1);
   }

   private byte m_140229_(net.minecraft.world.level.ChunkPos chunkPosIn, ChunkType chunkTypeIn) {
   ...
   public CompletableFuture<ChunkAccess> m_338637_(GenerationChunkHolder holderIn, ChunkStep chunkStepIn, StaticCache2D<GenerationChunkHolder> cacheIn) {
      net.minecraft.world.level.ChunkPos chunkpos = holderIn.m_338581_();
      if (chunkStepIn.f_337059_() == ChunkStatus.f_314297_) {
         ...
         try {
            ...
            var8.getStackTrace();
            net.minecraft.CrashReport crashreport = net.minecraft.CrashReport.m_127521_(var8, "Exception generating new chunk");
            ...
            crashreportcategory.m_128159_("Position hash", net.minecraft.world.level.ChunkPos.m_45589_(chunkpos.f_45578_, chunkpos.f_45579_));
   ...
   public ChunkGenerationTask m_338350_(ChunkStatus statusIn, net.minecraft.world.level.ChunkPos posIn) {
   ...
   public CompletableFuture<ChunkResult<LevelChunk>> m_143109_(ChunkHolder chunkHolderIn) {
      return this.m_280541_(chunkHolderIn, 1, net.minecraft.server.level.ChunkLevel::m_339977_)
         .thenApplyAsync(
   ...
   private boolean m_140425_(net.minecraft.world.level.ChunkPos chunkPosIn) {
   ...
   protected void m_293413_(int distanceIn) {
      int i = net.minecraft.util.Mth.m_14045_(distanceIn, 2, 64);
      if (i != this.f_290679_) {
         this.f_290679_ = i;
   ...
   int m_294650_(ServerPlayer playerIn) {
      return net.minecraft.util.Mth.m_14045_(playerIn.m_295486_(), 2, this.f_290679_);
   }

   private void m_294841_(ServerPlayer playerIn, net.minecraft.world.level.ChunkPos posIn) {
   ...
   private static void m_295122_(ServerPlayer playerIn, net.minecraft.world.level.ChunkPos posIn) {
   ...
   void m_140274_(Writer writerIn) throws IOException {
      ...
      while (var4.hasNext()) {
         ...
         net.minecraft.world.level.ChunkPos chunkpos = new net.minecraft.world.level.ChunkPos(i);
   ...
   private CompletableFuture<Optional<CompoundTag>> m_214963_(net.minecraft.world.level.ChunkPos chunkPosIn) {
      return this.m_223454_(chunkPosIn).thenApplyAsync(tagIn -> tagIn.map(this::m_214947_), net.minecraft.Util.m_183991_());
   }
   ...
   boolean m_183879_(net.minecraft.world.level.ChunkPos chunkPosIn) {
   ...
   public List<ServerPlayer> m_183888_(net.minecraft.world.level.ChunkPos chunkPosIn) {
   ...
   private boolean m_183751_(ServerPlayer playerIn, net.minecraft.world.level.ChunkPos chunkPosIn) {
      if (playerIn.m_5833_()) {
         return false;
      } else {
   ...
   public void m_140184_(ServerPlayer player) {
      ...
      while (sectionpos.hasNext()) {
         net.minecraft.server.level.ChunkMap.TrackedEntity chunkmap$trackedentity = (net.minecraft.server.level.ChunkMap.TrackedEntity)sectionpos.next();
   ...
   private void m_183754_(ServerPlayer player) {
      net.minecraft.world.level.ChunkPos chunkpos = player.m_146902_();
   ...
   public List<ServerPlayer> m_183262_(net.minecraft.world.level.ChunkPos pos, boolean boundaryOnly) {
      ...
      if (!multipart) {
         ...
         if (i != 0) {
            ...
            if (this.f_140150_.containsKey(entityIn.m_19879_())) {
               throw (IllegalStateException)net.minecraft.Util.m_137570_(new IllegalStateException("Entity is already tracked!"));
            }

            net.minecraft.server.level.ChunkMap.TrackedEntity chunkmap$trackedentity = new net.minecraft.server.level.ChunkMap.TrackedEntity(
               entityIn, i, j, entitytype.m_20683_()
            );
            ...
            if (entityIn instanceof ServerPlayer serverplayer) {
               ...
               while (var8.hasNext()) {
                  net.minecraft.server.level.ChunkMap.TrackedEntity chunkmap$trackedentity1 = (net.minecraft.server.level.ChunkMap.TrackedEntity)var8.next();
   ...
   protected void m_140331_(Entity entityIn) {
      if (entityIn instanceof ServerPlayer serverplayer) {
         ...
         while (var3.hasNext()) {
            net.minecraft.server.level.ChunkMap.TrackedEntity chunkmap$trackedentity = (net.minecraft.server.level.ChunkMap.TrackedEntity)var3.next();
            chunkmap$trackedentity.m_140485_(serverplayer);
         }
      }

      net.minecraft.server.level.ChunkMap.TrackedEntity chunkmap$trackedentity1 = (net.minecraft.server.level.ChunkMap.TrackedEntity)this.f_140150_
         .remove(entityIn.m_19879_());
   ...
   protected void m_140421_() {
      ...
      if (!list.isEmpty()) {
         ...
         while (var3.hasNext()) {
            net.minecraft.server.level.ChunkMap.TrackedEntity chunkmap$trackedentity1 = (net.minecraft.server.level.ChunkMap.TrackedEntity)var3.next();
   ...
   public void m_140201_(Entity entityIn, Packet<?> packetIn) {
      net.minecraft.server.level.ChunkMap.TrackedEntity chunkmap$trackedentity = (net.minecraft.server.level.ChunkMap.TrackedEntity)this.f_140150_
         .get(entityIn.m_19879_());
   ...
   public void m_274524_(List<ChunkAccess> chunksIn) {
      ...
      for (ChunkAccess chunkaccess : chunksIn) {
         net.minecraft.world.level.ChunkPos chunkpos = chunkaccess.m_7697_();
   ...
   void m_287285_(net.minecraft.world.level.ChunkPos chunkPosIn, FullChunkStatus statusIn) {
      this.f_143031_.m_156794_(chunkPosIn, statusIn);
   }

   public void m_293872_(net.minecraft.world.level.ChunkPos chunkPosIn, int distIn) {
      int i = distIn + 1;
      net.minecraft.world.level.ChunkPos.m_45596_(chunkPosIn, i).forEach(chunkPos2In -> {
   ...
   class TrackedEntity {
      ...
      public boolean equals(Object p_equals_1_) {
         return p_equals_1_ instanceof net.minecraft.server.level.ChunkMap.TrackedEntity
            ? ((net.minecraft.server.level.ChunkMap.TrackedEntity)p_equals_1_).f_140472_.m_19879_() == this.f_140472_.m_19879_()
            : false;
      }
      ...
      public void m_140497_(ServerPlayer player) {
         if (player != this.f_140472_) {
            net.minecraft.world.phys.Vec3 vec3 = player.m_20182_().m_82546_(this.f_140472_.m_20182_());
