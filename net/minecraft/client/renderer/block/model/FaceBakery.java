...
public class FaceBakery {
   public static int f_173433_;
   private static float f_111569_ = 1.0F / (float)Math.cos((float) (Math.PI / 8)) - 1.0F;
   private static float f_111570_ = 1.0F / (float)Math.cos((float) (Math.PI / 4)) - 1.0F;
   public static int f_173434_;
   private static int f_173436_;
   public static int f_173435_;

   public BakedQuad m_111600_(
      Vector3f posFrom,
      Vector3f posTo,
      ...
      if (Reflector.ForgeHooksClient_fillNormal.exists() && Reflector.ForgeBlockElementFace_getFaceData.exists()) {
         ForgeFaceData data = (ForgeFaceData)Reflector.m_46374_(face, Reflector.ForgeBlockElementFace_getFaceData);
         ...
         if (!ForgeFaceData.DEFAULT.equals(data)) {
            Object iQuadTransformerLM = Reflector.QuadTransformers_applyingLightmap.m_46374_(data.blockLight(), data.skyLight());
            if (iQuadTransformerLM != null) {
               Reflector.m_46374_(iQuadTransformerLM, Reflector.IQuadTransformer_processInPlace, quad);
            }

            Object iQuadTransformerCol = Reflector.QuadTransformers_applyingColor.m_46374_(data.color());
            if (iQuadTransformerCol != null) {
               Reflector.m_46374_(iQuadTransformerCol, Reflector.IQuadTransformer_processInPlace, quad);
            }
         }
   ...
   public static BlockFaceUV m_111581_(BlockFaceUV blockFaceUVIn, Direction facing, Transformation modelRotationIn) {
      ...
      float f2 = 16.0F * vector4f.m_305649_();
      float f3 = 16.0F * vector4f.m_306225_();
      ...
      float f6 = 16.0F * vector4f1.m_305649_();
      float f7 = 16.0F * vector4f1.m_306225_();
      float f8;
      float f9;
      ...
      int i = Math.floorMod(-((int)Math.round(Math.toDegrees(Math.atan2((double)vector3f.m_306225_(), (double)vector3f.m_305649_())) / 90.0)) * 90, 360);
   ...
   private float[] m_111592_(Vector3f pos1, Vector3f pos2) {
      ...
      afloat[Constants.f_108996_] = pos1.m_305649_() / 16.0F;
      afloat[Constants.f_108995_] = pos1.m_306225_() / 16.0F;
      afloat[Constants.f_108994_] = pos1.m_240700_() / 16.0F;
      afloat[Constants.f_108993_] = pos2.m_305649_() / 16.0F;
      afloat[Constants.f_108992_] = pos2.m_306225_() / 16.0F;
      afloat[Constants.f_108991_] = pos2.m_240700_() / 16.0F;
      return afloat;
   }
   ...
   private void m_111614_(int[] faceData, int storeIndex, Vector3f positionIn, TextureAtlasSprite sprite, BlockFaceUV faceUV) {
      ...
      faceData[i] = Float.floatToRawIntBits(positionIn.m_305649_());
      faceData[i + 1] = Float.floatToRawIntBits(positionIn.m_306225_());
      faceData[i + 2] = Float.floatToRawIntBits(positionIn.m_240700_());
      faceData[i + 3] = -1;
   ...
   private void m_252985_(Vector3f vec, @Nullable BlockElementRotation partRotation) {
      if (partRotation != null) {
         Vector3f vector3f;
         Vector3f vector3f1;
         switch (<unrepresentable>.$SwitchMap$net$minecraft$core$Direction$Axis[partRotation.f_111379_().ordinal()]) {
            case 1:
         ...
         this.m_252821_(vec, new Vector3f(partRotation.f_111378_()), new Matrix4f().m_252961_(quaternionf), vector3f1);
      }
   }
   ...
   private void m_252821_(Vector3f posIn, Vector3f originIn, Matrix4f transformIn, Vector3f scaleIn) {
      Vector4f vector4f = transformIn.transform(
         new Vector4f(posIn.m_305649_() - originIn.m_305649_(), posIn.m_306225_() - originIn.m_306225_(), posIn.m_240700_() - originIn.m_240700_(), 1.0F)
      );
      vector4f.mul(new Vector4f(scaleIn, 1.0F));
      posIn.set(vector4f.m_305649_() + originIn.m_305649_(), vector4f.m_306225_() + originIn.m_306225_(), vector4f.m_240700_() + originIn.m_240700_());
   }
