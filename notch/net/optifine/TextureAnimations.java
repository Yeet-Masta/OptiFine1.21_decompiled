...
public class TextureAnimations {
   ...
   private static TextureAnimation[] getTextureAnimations(C_50_[] rps) {
   ...
   private static TextureAnimation[] getTextureAnimations(C_50_ rp) {
      ...
      if (animPropNames.length <= 0) {
         ...
         for (int i = 0; i < animPropNames.length; i++) {
            ...
            try {
               C_5265_ propLoc = new C_5265_(propName);
               InputStream in = Config.getResourceStream(rp, C_51_.CLIENT_RESOURCES, propLoc);
               Properties props = new PropertiesOrdered();
               props.load(in);
               in.close();
               TextureAnimation anim = makeTextureAnimation(props, propLoc);
               if (anim != null) {
                  C_5265_ locDstTex = new C_5265_(anim.getDstTex());
   ...
   private static TextureAnimation makeTextureAnimation(Properties props, C_5265_ propLoc) {
      ...
      if (texFrom == null || texTo == null) {
         ...
         if (imageBytes == null) {
            ...
            if (countPixels != countPixelsAllFrames) {
               ...
               C_5265_ locTexTo = new C_5265_(texTo);
   ...
   private static byte[] loadImage(String name, int targetWidth) {
      C_3401_ options = Config.getGameSettings();

      try {
         C_5265_ locRes = new C_5265_(name);
