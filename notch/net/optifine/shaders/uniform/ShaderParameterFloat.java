   ...
   public float eval(int index1, int index2) {
      if (this.indexNames1 == null || index1 >= 0 && index1 <= this.indexNames1.length) {
         if (this.indexNames2 == null || index2 >= 0 && index2 <= this.indexNames2.length) {
            switch (this) {
               case BIOME: {
                  C_4675_ pos = Shaders.getWorldCameraPosition();
                  C_203228_<C_1629_> biome = Shaders.getCurrentWorld().t(pos);
                  return (float)BiomeUtils.getId((C_1629_)biome.m_203334_());
               }
               case BIOME_CATEGORY: {
                  C_4675_ pos = Shaders.getWorldCameraPosition();
                  C_203228_<C_1629_> biome = Shaders.getCurrentWorld().t(pos);
               ...
               case BIOME_PRECIPITATION: {
                  C_4675_ pos = Shaders.getWorldCameraPosition();
                  C_203228_<C_1629_> biome = Shaders.getCurrentWorld().t(pos);
                  return biome != null ? (float)BiomeUtils.getPrecipitation((C_1629_)biome.m_203334_()).ordinal() : 0.0F;
               }
               case TEMPERATURE: {
                  C_4675_ pos = Shaders.getWorldCameraPosition();
                  C_203228_<C_1629_> biome = Shaders.getCurrentWorld().t(pos);
                  return biome != null ? ((C_1629_)biome.m_203334_()).m_47554_() : 0.0F;
               }
               case RAINFALL: {
                  C_4675_ pos = Shaders.getWorldCameraPosition();
                  C_203228_<C_1629_> biome = Shaders.getCurrentWorld().t(pos);
                  return biome != null ? BiomeUtils.getDownfall((C_1629_)biome.m_203334_()) : 0.0F;
               }
               default:
