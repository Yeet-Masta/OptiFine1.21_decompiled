...
public class ShadersTex {
   ...
   public static MultiTexID getMultiTexID(C_4468_ tex) {
   ...
   public static void deleteTextures(C_4468_ atex, int texid) {
   ...
   public static void bindTexture(int id) {
      C_4468_ tex = Config.getTextureManager().getTextureById(id);
      if (tex == null) {
   ...
   public static void bindTexture(C_4468_ tex) {
      ...
      if (GlStateManager.getActiveTextureUnit() == 33984) {
         ...
         if (tex instanceof C_4484_) {
            Shaders.atlasSizeX = ((C_4484_)tex).atlasWidth;
            Shaders.atlasSizeY = ((C_4484_)tex).atlasHeight;
         } else {
   ...
   public static void initDynamicTextureNS(C_4470_ tex) {
      MultiTexID multiTex = tex.getMultiTexID();
      C_3148_ nativeImage = tex.m_117991_();
      int width = nativeImage.m_84982_();
      int height = nativeImage.m_85084_();
      C_3148_ imageNormal = makeImageColor(width, height, -8421377);
      TextureUtil.prepareImage(multiTex.norm, width, height);
      imageNormal.m_85013_(0, 0, 0, 0, 0, width, height, false, false, false, true);
      C_3148_ imageSpecular = makeImageColor(width, height, 0);
   ...
   public static C_4468_ createDefaultTexture() {
      C_4470_ tex = new C_4470_(1, 1, true);
      tex.m_117991_().m_84988_(0, 0, -1);
      tex.m_117985_();
      return tex;
   }

   public static void allocateTextureMapNS(int mipmapLevels, int width, int height, C_4484_ tex) {
   ...
   private static C_3148_[] generateMipmaps(C_3148_ image, int levels) {
      if (levels < 0) {
         levels = 0;
      }

      C_3148_[] mipmapImages = new C_3148_[levels + 1];
      mipmapImages[0] = image;
      if (levels > 0) {
         for (int level = 1; level <= levels; level++) {
            C_3148_ imageParent = mipmapImages[level - 1];
            C_3148_ imageChild = new C_3148_(imageParent.m_84982_() >> 1, imageParent.m_85084_() >> 1, false);
   ...
   public static BufferedImage readImage(C_5265_ resLoc) {
      try {
   ...
   public static C_5265_ getNSMapLocation(C_5265_ location, String mapName) {
      if (location == null) {
         return null;
      } else {
         ...
         return new C_5265_(location.m_135827_(), basenameNoFileType + "_" + mapName + ".png");
      }
   }

   private static C_3148_ loadNSMapImage(C_77_ manager, C_5265_ location, int width, int height, int defaultColor) {
      C_3148_ image = loadNSMapFile(manager, location, width, height);
      if (image == null) {
         image = new C_3148_(width, height, false);
   ...
   private static C_3148_ makeImageColor(int width, int height, int defaultColor) {
      C_3148_ image = new C_3148_(width, height, false);
      int colAbgr = TextureUtils.toAbgr(defaultColor);
      image.m_84997_(0, 0, width, height, colAbgr);
      return image;
   }

   private static C_3148_ loadNSMapFile(C_77_ manager, C_5265_ location, int width, int height) {
      if (location == null) {
         return null;
      } else {
         try {
            C_76_ res = manager.getResourceOrThrow(location);
            C_3148_ image = C_3148_.m_85058_(res.m_215507_());
            if (image == null) {
               return null;
   ...
   public static void loadSimpleTextureNS(
      int textureID, C_3148_ nativeImage, boolean blur, boolean clamp, C_77_ resourceManager, C_5265_ location, MultiTexID multiTex
   ) {
      int width = nativeImage.m_84982_();
      int height = nativeImage.m_85084_();
      C_5265_ locNormal = getNSMapLocation(location, "n");
      C_3148_ imageNormal = loadNSMapImage(resourceManager, locNormal, width, height, -8421377);
      TextureUtil.prepareImage(multiTex.norm, 0, width, height);
      imageNormal.m_85013_(0, 0, 0, 0, 0, width, height, blur, clamp, false, true);
      C_5265_ locSpecular = getNSMapLocation(location, "s");
      C_3148_ imageSpecular = loadNSMapImage(resourceManager, locSpecular, width, height, 0);
   ...
   public static void updateTextureMinMagFilter() {
      C_4490_ texman = C_3391_.m_91087_().m_91097_();
      C_4468_ texObj = texman.m_118506_(C_4484_.f_118259_);
      if (texObj != null) {
   ...
   public static int[][] prepareAF(C_4486_ tas, int[][] src, int width, int height) {
      boolean skip = true;
      return src;
   }

   public static void fixTransparentColor(C_4486_ tas, int[] aint) {
   }
}
