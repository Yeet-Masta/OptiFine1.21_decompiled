...
public class NaturalProperties {
   ...
   public synchronized C_4196_ getQuad(C_4196_ quadIn, int rotate, boolean flipU) {
      int index = rotate;
      if (flipU) {
         index = rotate | 4;
      }
      ...
      if (index > 0 && index < this.quadMaps.length) {
         ...
         C_4196_ quad = (C_4196_)map.get(quadIn);
         if (quad == null) {
   ...
   private C_4196_ makeQuad(C_4196_ quad, int rotate, boolean flipU) {
      ...
      return new C_4196_(vertexData, tintIndex, face, sprite, shade);
   }
   ...
   private boolean isFullSprite(C_4196_ quad) {
      C_4486_ sprite = quad.m_173410_();
   ...
   private boolean equalsDelta(float x1, float x2, float deltaMax) {
      float deltaAbs = C_188_.m_14154_(x1 - x2);
