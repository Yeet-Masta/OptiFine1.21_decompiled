...
public abstract class C_4357_<T extends C_524_, M extends C_3819_<T>> extends C_4331_<T> implements C_4382_<T, M> {
   private static final Logger f_115289_ = LogUtils.getLogger();
   private static final float f_174287_ = 0.1F;
   public M f_115290_;
   protected final List<C_4447_<T, M>> f_115291_ = Lists.newArrayList();
   ...
   public C_4357_(C_141743_ contextIn, M entityModelIn, float shadowSizeIn) {
      super(contextIn);
      this.f_115290_ = entityModelIn;
      this.f_114477_ = shadowSizeIn;
   }

   public final boolean m_115326_(C_4447_<T, M> layer) {
   ...
   public void m_7392_(T entityIn, float entityYaw, float partialTicks, C_3181_ matrixStackIn, C_4139_ bufferIn, int packedLightIn) {
      if (!Reflector.RenderLivingEvent_Pre_Constructor.exists()
         ...
         float f2 = f1 - f;
         if (this.f_115290_.f_102609_ && entityIn.bS() && entityIn.dc() instanceof C_524_ livingentity) {
            f = C_188_.m_14189_(partialTicks, livingentity.f_20884_, livingentity.f_20883_);
            f2 = f1 - f;
            float f7 = C_188_.m_14177_(f2);
            if (f7 < -85.0F) {
               f7 = -85.0F;
            }
         ...
         if (entityIn.c(C_535_.SLEEPING)) {
            ...
            if (direction != null) {
               float f3 = entityIn.d(C_535_.STANDING) - 0.1F;
         ...
         if (!entityIn.R_()) {
            for (C_4447_<T, M> renderlayer : this.f_115291_) {
   ...
   @Nullable
   protected C_4168_ m_7225_(T entityIn, boolean visibleIn, boolean translucentIn, boolean glowingIn) {
   ...
   protected void m_7523_(T entityLiving, C_3181_ matrixStackIn, float ageInTicks, float rotationYaw, float partialTicks, float renderScale) {
      if (this.m_5936_(entityLiving)) {
         rotationYaw += (float)(Math.cos((double)entityLiving.ai * 3.25) * Math.PI * 0.4F);
      }
   ...
   protected void m_7546_(T entitylivingbaseIn, C_3181_ matrixStackIn, float partialTickTime) {
   }
   ...
   public <T extends C_4447_> T getLayer(Class<T> cls) {
      List<T> list = this.getLayers(cls);
      return (T)(list.isEmpty() ? null : list.get(0));
   }

   public <T extends C_4447_> List<T> getLayers(Class<T> cls) {
      List<C_4447_> list = new ArrayList();

      for (C_4447_ layer : this.f_115291_) {
